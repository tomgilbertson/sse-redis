...
logs are added to our log file. It will also publish them to Redis.
"""
...
# Anytime the generator detects a new line in the log file, it will yield it and publish to Redis.
async def logGenerator(request):
    for line in tail("-f", LOGFILE, _iter=True):
        if await request.is_disconnected():
            ...
            break
        yield line
        # try and add to redis
        red.publish('name_of_channel', line)
        # time.sleep(0.5)
